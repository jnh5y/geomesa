    ...
    if (ranges.isEmpty) EmptyPlan(filter)
      ...
      if (ranges.head.isInstanceOf[Get]) {
        ...
        if(ds.config.isBigtable) {
          ...
          hbaseScanPlan(filter, ranges, table, hbaseFilters, coprocessor, hints, toFeatures)
        }
      }
    }
  }

  private def hbaseScanPlan(filter: HBaseFilterStrategyType,
                            ranges: Seq[Query],
                            table: TableName,
    ...
    coprocessor match {
      case Some(processor) =>
        DensityCoprocessorPlan(filter, hints, table, ranges.asInstanceOf[Seq[Scan]], hbaseFilters, toFeatures)
      case None =>
  ...
  protected def scanConfig(sft: SimpleFeatureType,
    ...
    val coprocessor: Option[Coprocessor] = {
      if (hints.isDensityQuery) {
        println("CONFIGURING Density Coprocessor")
