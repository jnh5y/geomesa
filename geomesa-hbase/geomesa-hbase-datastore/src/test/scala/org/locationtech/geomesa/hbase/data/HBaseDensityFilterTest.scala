...
import org.locationtech.geomesa.hbase.utils.{HBaseBatchScan, KryoLazyDensityFilterUtils}
...
//      var q = new Query(typeName, ECQL.toFilter("(dtg between '2012-01-01T18:00:00.000Z' AND '2012-01-01T23:00:00.000Z') and BBOX(geom, -80, 33, -70, 40)"))
...
//       }
    ...
    def getDensity(query: String, fs: SimpleFeatureStore): List[(Double, Double, Double)] = {
      ...
      q.getHints.put(QueryHints.DENSITY_WIDTH, 500)
      q.getHints.put(QueryHints.DENSITY_HEIGHT, 500)
      val decode = KryoLazyDensityFilterUtils.decodeResult(env, 500, 500)
